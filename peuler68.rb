...
outside, middle, inside, pentagon, answers = [],[],[],[],[]
pentagoni = 0

def ismagic?(arr)
	sum = arr[0].reduce(:+)
	arr.each {|a| return false if a.reduce(:+) != sum}
	true
end

(6..10).to_a.permutation {|o|
	outside = o
	until outside[0] == outside.min
		outside.rotate!
	end
	(1..5).to_a.permutation {|m|
		middle = m
		inside = middle.rotate
		pentagon = [outside,middle,inside].transpose
		next unless ismagic?(pentagon)
		pentagoni = pentagon.flatten.collect{|d|d.to_s}.reduce(:+).to_i
		answers.push pentagoni if (! answers.include?(pentagoni))
	}
}


puts answers
puts "the answer is #{answers.max}"